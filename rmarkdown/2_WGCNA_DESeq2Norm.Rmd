...
```
We identified 15 different cutting heights on the hierarchical clustering tree including the height that does not remove any samples. These chosen cutting heights and their corresponding number of identified outliers and the list of outliers are shown in `r table_nums_1("wgcna_cutheights_table_1",display="cite")`. 
```{r}
...
Based on the comparison in `r figure_nums_1("wgcna_compare_unadjusted",display="cite")`, we decided to use the cutting height= which defined 15 outliers for the WGCNA analysis. The identified gene modules and their correlation with the given clinical trais are shown in .
...
output.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/WGCNA/WGCNA_DESeq2Norm_original")
rdata.filepath<-file.path(output.dir,"wgcna_cutheights.RData")
sftpower.filepath<-file.path(output.dir,"sft_power_list.txt")
load(rdata.filepath)
...
modules.chosen<-rownames(moduleTraitPvalue)[moduleTraitPvalue[,"AGE"]<0.05]
...
labeledHeatmap(Matrix = t(as.matrix(temp.moduleTraitCor)),
xLabels = rownames(my.datTraits[]),
...
cex.text = 0.5,
zlim = c(-1,1),
main = paste("Module-trait relationships (AGE)"),
naColor="white")
#savePlot(output.filepath,type="jpeg")
```

2. Gender is correlated with a few gene modules, which are also correlated with BDFVC, BDFEV1 in liters, suggesting the gender effect on the unadjusted PFTs again.
...
3. RACE is also correlated with a few gene modules, which are also correlated with PFTs in liters.
...
#savePlot(output.filepath,type="jpeg")
```
4. There are some gene modules that are correlated with many PFTs. Some of them are also correlated with GENDER, RACE. But some of them are also correlated with PFTs independently of GENDER and RACE.
...
# WGCNA on PBMC baseline (original log2 transformed)

In this section, we run WGCNA on the log2 transformed DESeq2 normalized data. 

Save the following code block as R file to load in and match the clinical data, fpkm matrix, and the sample mapping matrix.
```{r 0_CleanDataLoad_DESeq2Norm_log2original.R,eval=FALSE,echo=TRUE,results='hide'}
# /home/xy48/Rprogram/GRADS_PBMC/WGCNA/0_CleanDataLoad.R
home.dir<-"/home/yanxiting/driver_Grace"
#home.dir<-"/home/xy48"
data.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38")
...
fpkm.matrix<-log(fpkm.matrix+1,base=2)
data.matrix<-fpkm.matrix
...
Load in clinical matrix and extract data for the variables needed to correlate with the gene modules. Some of the clinical featuers were also re-coded.
```{r 6_0_DataLoad_DESeq2Norm_log2original.R,eval=FALSE,echo=TRUE,results='hide'}
#-------------------------------------------------------------------------------------------------# /Rprogram/GRADS_PBMC/WGCNA/6_0_DataLoad.R
###################################################################
#  Load in both the gene expression and the clinical data
#  filter the genes based on given cv.threshold and match the gene expression data and the clinical data
#home.dir<-"/home/xy48"
home.dir<-"/home/yanxiting/driver_Grace"
source(file.path(home.dir,"Rprogram/GRADS_PBMC/WGCNA/0_CleanDataLoad_DESeq2Norm_log2original.R"))
output.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/WGCNA/WGCNA_DESeq2Norm_log2original")
...
We need to generate the hclust_cut_heights_list.txt to provide the heights at which we would like to cut the hierarchical clustering tree. Name of this file cannot be changed and it has to be saved under the same folder where all WGCNA results are saved.
```{r 6_2_sftpicking_DESeq2Norm_log2original.R,eval=FALSE,echo=TRUE,results='hide'}
#----------------------------------------------------------------------------------------------------------------------------------------------------
# /Rprogram/GRADS/BAL_final_hg38_20180726/WGCNA_unadjusted_ALL_filtering/6_2_sftpicking.R
####################################################
# load in all cut heights for a given cv.threshold, generate the soft power picking figures for each cut height
...
We need to review the plots in sft_picking.pdf and generate a text file with each line describing the softpower we want to set for each cut height we picked before. This text file will be saved as sft_power_list.txt. The numbers will be in the same order as those plots in the sft_picking.pdf. 
```{r 6_3_clustering_DESeq2Norm_log2original.R,eval=FALSE,echo=TRUE,results='hide'}
#---------------------------------------------------------------------------------------------------------------------------------------------------
# /Users/yanxiting/MyVolumes/GRACE/Rprogram/GRADS/BAL_final_hg38_20180726/WGCNA_unadjusted_ALL_filtering/6_3_clustering.R
#output.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/CV0_compare_V2")
#rdata.filepath<-file.path(output.dir,"sft_picking.RData")
...
# WGCNA on PBMC baseline (adjusted)

First, we adjusted the DESeq2 normalized data to remove the cell differential effects.
```{r eval=FALSE}
######################################################
...
#rownames(data.matrix)<-paste(anno.matrix[,"Gene_Id"],"_",anno.matrix[,"Chr"],sep="")

# load in the clinical data matrix
#clinic.matrix=read.table(clinic.filepath,header=T,sep="\t",check.names=F,stringsAsFactors=F,quote = "")
clinic.matrix<-read.csv(clinic.filepath,check.names=F,header=T,as.is=TRUE)
rownames(clinic.matrix)<-as.matrix(clinic.matrix)[,1] # GRADS ID
clinic.matrix<-clinic.matrix[colnames(data.matrix),]

my.data.matrix<-data.matrix
collectGarbage()
...
#my.data.matrix<-my.data.matrix[,!is.na(cell.data[,2])]
my.data.matrix<-my.data.matrix[,apply(is.na(cell.data),1,sum)==0]
cell.data<-cell.data[apply(is.na(cell.data),1,sum)==0,]

# make sure all the cell differentials add up to 100
#temp.matrix<-apply(cell.data,1,sum)
#temp.matrix<-matrix(rep(temp.matrix,ncol(cell.data)),nrow=nrow(cell.data),byrow=F)
#cell.data<-cell.data*100/temp.matrix

# get rid of the neutrophil and adjust the other cell differentials to sum up to 100%
cell.data.orig<-cell.data
cell.data<-cell.data[,colnames(cell.data)!="p_neut"]
temp.matrix<-matrix(rep(apply(cell.data,1,sum),ncol(cell.data)),nrow=nrow(cell.data),byrow=F)
cell.data<-cell.data*80/temp.matrix

# adjust the data
res.matrix<-numeric()
for(i in 1:nrow(my.data.matrix)){
  temp.data<-data.frame(gexp=t(my.data.matrix[i,]),mono=cell.data[,1],eos=cell.data[,2],lym=cell.data[,3])
...
output.filepath<-file.path(output.dir,"DESeq2_normalized_276_clean_celldiffadjusted_withannot.txt")
write.table(cmd.out,file=output.filepath,append=F,sep="\t",row.names=F,col.names=T,quote=F)

```

In this section, we run WGCNA on the cell differential adjusted DESeq2 normalized data. No log2 transformation was done.
...
home.dir<-"/home/yanxiting/driver_Grace"
source(file.path(home.dir,"Rprogram/GRADS_PBMC/WGCNA/0_CleanDataLoad_DESeq2Norm_log2original.R"))
output.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/WGCNA/WGCNA_DESeq2Norm_original_adjusted")
...
# WGCNA on PBMC baseline (adjusted log2 transformed)
...
data.matrix<-log(data.matrix+1,base=2)
#rownames(data.matrix)<-paste(anno.matrix[,"Gene_Id"],"_",anno.matrix[,"Chr"],sep="")
...
output.filepath<-file.path(output.dir,"DESeq2_normalized_276_clean_log2_celldiffadjusted_withannot.txt")
write.table(cmd.out,file=output.filepath,append=F,sep="\t",row.names=F,col.names=T,quote=F)

```

In this section, we run WGCNA on the cell differential adjusted DESeq2 normalized data log2 transformation was done.
...
data.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38")
#fpkm.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/data","DESeq2_normalized_276_clean.txt")
fpkm.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/data_adjusted2","DESeq2_normalized_276_clean_log2_celldiffadjusted_withannot.txt")
masterkey.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Data/GRADS Master Progress Key 6-13-16.xlsx")
...
#home.dir<-"/home/yanxiting/driver_Grace"
#source(file.path(home.dir,"Rprogram/GRADS_PBMC/WGCNA/0_CleanDataLoad_DESeq2Norm_log2original.R"))
output.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/WGCNA/WGCNA_DESeq2Norm_log2original_adjusted")
