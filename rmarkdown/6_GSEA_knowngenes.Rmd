...
We downloaded a previously published 10X Genomicx data and compared the genes in Bloom data to see if they are mostly cell type markers.

```{r}
# load in the scRNA-seq data of SARC PBMC samples that was previously published
data.filepath<-"~/driver_Grace/scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/data/GSE132338_SingleCellExperiment.RDS"
sc.data<-readRDS(data.filepath,refhook=NULL)

# identify the cell type marker genes
Idents(sc.data)<-sc.data@meta.data$celltype
celltype.names<-unique(as.character(sc.data@meta.data$celltype))
sc.markers.result<-list()

for(i in 1:length(celltype.names)){
  sc.markers.result[[i]]<-FindMarkers(sc.data,ident.1=celltype.names[i],min.pct=0.05)
}
