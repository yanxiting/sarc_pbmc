...
# Style 1 (HPC)
This style will be run on a hpc and specifically in this note, it will be run on grace hpc. 
...
data.dir.list<-c("/home/xy48/scratch/Shervin/data/sample_dir_000005659",
                 "/home/xy48/scratch/Shervin/data/sample_dir_000005540")
...
sample.names<-list.dirs(data.dir,full.names = FALSE, recursive=FALSE)
...
for(i in 1:length(sample.names)){
  ...
  fastq.filepath<-list.files(file.path(data.dir,sample.names[i]),full.names = TRUE)
...
The code chunck above will generate one sh file for each sample and a jobsub.bat file to submit all the jobs to the hpc. We just need to run jobsub.bat as an executable file to start all the jobs.

# Style 2 (Workstation)
Since each FastQC won't take too much time, when there are not so many samples, it is more convenient for us to run FastQC on different samples in one single thread on a workstation. 

```{r}
# this chunck needs to be run on the workstation.
data.dir.list<-c(file.path(home.dir,"scratch/Shervin/data/sample_dir_000005659"),
            file.path(home.dir,"scratch/Shervin/data/sample_dir_000005540"))

work.dir<-file.path(home.dir,"scratch/Shervin")
fastqc.filepath<-"/home/yanxiting/MyTools/FastQC/fastqc" # This has to be a location on the workstation.

script.filepath<-"/home/yanxiting/Documents/Research/Shervin/FastQC.bat"
log.filepath<-"/home/yanxiting/Documents/Research/Shervin/FastQC.log"

if(file.exists(script.filepath)){
  file.remove(script.filepath)
}
file.create(script.filepath)
...
for(data.dir in data.dir.list){
  ...
  fastq.filepath<-list.files(file.path(data.dir,sample.names[i]),full.names = TRUE)
  temp<-paste(fastqc.filepath," -o ",output.subdir," -t 15 ",fastq.filepath,"\n",sep="")
  cat(temp,file=script.filepath,append=T)
}
...
We run the FastQC.bat using `nohup FastQC.bat > FastQC.log &`. It's better to run this in shell because the screen output can be redirected into the log file. Directly running this within rstudio will cause the screen output present in rstudio screen.
