...
First, we use all samples except X235 based on the PCA analysis.

### Identify DEGs

We first identify DEGs using DESeq2.

```{r}
library(DESeq2)
library(apeglm)
source(file.path(home.dir,"Rprogram/my_functions.R"))
count.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth","rawcounts_all.txt")
clinic.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/data/cxr_merge_ucsf_seq.xlsx")
output.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/DEGs/DESeq2")
...
count.matrix<-count.matrix[,colnames(count.matrix)!="sarcoidosis_X235"]
count.matrix<-count.matrix[,colnames(count.matrix)!="duplicate_X118"]

# loading the clinic data and match it with the raw count matrix
clinic.matrix<-read.xlsx(data.filepath,sheetIndex = 1,check.names=F,stringsAsFactors=FALSE)
#clinic.matrix<-readRDS(clinic.filepath,refhook = NULL)
rownames(clinic.matrix)<-clinic.matrix$sample_names

temp.names<-intersect(rownames(clinic.matrix),colnames(count.matrix))
clinic.matrix<-clinic.matrix[temp.names,]
count.matrix<-count.matrix[,temp.names]
...
my.coldata<-my.coldata[my.coldata$CXR_stage%in%c(1,2,3,4),]
my.coldata$CXR_stage[my.coldata$CXR_stage%in%c(2,3,4)]<-2
my.coldata$CXR_stage<-factor(my.coldata$CXR_stage,levels=c(1,2))
my.coldata$CXR_stage <- relevel(my.coldata$CXR_stage, ref = 1)
...
dds<-DESeq(dds)
res<-results(dds)
resLFC <- lfcShrink(dds, coef="CXR_stage_2_vs_1", type="apeglm")
...
We over the identified DEGs with BLOOM gene list.

```{r}
bloom.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/GSEA_knowngenes/genesetinflammatory.gmx.txt")
bloom.genelist<-read.table(bloom.filepath,sep="\t",header=T,check.names=F,stringsAsFactors = F)

grads.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/DEGs/DESeq2/DEGs_fdr0.05_PHENGRP_Stage.II.IV.untreated_vs_Non.acute.Stage.I.untreated.csv")
grads.degs<-read.csv(grads.filepath)

ucsf.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/DEGs/DESeq2/DEGs_all_CXR_stage_2_vs_1.csv")
ucsf.degs<-read.csv(ucsf.filepath)
ucsf.degs<-ucsf.degs[ucsf.degs$pvalue<0.05 & (ucsf.degs$log2FoldChange>1 | ucsf.degs$log2FoldChange< -1),]

overlap.genes<-intersect(as.character(grads.degs$GeneID),bloom.genelist$BLOOM)
...
Second, we use samples without outliers based on the PCA analysis.
...
count.matrix<-count.matrix[,!colnames(count.matrix)%in%outlier.list]
