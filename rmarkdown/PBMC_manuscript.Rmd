...
# Introduction
This note contains all codes we used to generate the figures and tables in the manuscript including those in the supplemental materials. Codes conduting the actual analysis behind each figure and table will be cound in other notes.

# Table 1. Patient Characteristics

In this table, we show the summary of the patient characteristics for all patients included in the analysis.
...
clinic.data<-readRDS("/home/yanxiting/driver_Grace/scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/data/clinic_matrix_merged.RDS",refhook = NULL)
output.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/manuscript")
...
for(i in 1:length(var.names)){
  if(var.type[i]=="cat"){
    ...
    colnames(temp.matrix)<-colnames(temp)
    ...
    names(temp.out)<-names(temp.list)
    if(i>1){
...
output.filepath<-file.path(output.dir,"table1_XY.csv")
write.csv(cmd.out,file=output.filepath, append = F,row.names=T)

cat("Table 1 can be found in ",output.filepath,".\n",sep="")
```


# Codes from BAL paper(just for reference)
