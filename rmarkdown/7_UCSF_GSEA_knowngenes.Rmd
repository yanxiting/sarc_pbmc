...
my_gsea_filecreate_binary<-function(fpkm.matrix,clinic.matrix,fpkm.matrix.anno,var.names,group1.values,group2.values,group1.name="group1",group2.name="group2",output.dir,suffix.name){
  ...
  gct.filepath.2<-file.path(output.dir,paste("gexp_",suffix.name,"_withID.gct",sep=""))
  cls.filepath.2<-file.path(output.dir,paste("cls_",suffix.name,"_withID.cls",sep=""))
...
output.folder<-file.path(home.dir,"scratch60/Laura_Koth/GSEA_knowngenes_log2DESeq2/all_samples_withID")
...
temp<-my_gsea_filecreate_binary(fpkm.matrix=fpkm.data,
                          ...
                          fpkm.matrix.anno = anno.data,
                          var.names = varnames,
                          group1.values = data.frame(CXR_stage=c("0","1")),
                          group2.values = data.frame(PHENGRP=c("2","3","4")),
                          output.dir=output.folder,
                          suffix.name="analysis1c"
                          )
...
# note this function is slightly different for DESeq2 Norm data and the TPM matrix
#my_gsea_filecreate_binary<-function(fpkm.filepath,clinic.filepath,var.names,group1.values,group2.values,group1.name="group1",group2.name="group2",output.dir,suffix.name){
output.folder<-file.path(home.dir,"scratch60/Laura_Koth/GSEA_knowngenes_log2DESeq2/no_outliers_withID")
...
# Data adjustment for cell differentials

In this section, we adjust the gene expression data to remove cell differentials effects so that the GSEA input file can be generated using the adjusted data for all samples and samples without outliers. 

## DESeq2 Log2 (all samples)

First, we load in the gene expression data and the cell differential data. Only X235 was excluded.
...
deseq2.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth","DESeq2_normalized.RDS")
my.data<-readRDS(deseq2.filepath,refhook = NULL)
...
Second, we load in the cell differential data.

```{r}
home.dir<-"/home/yanxiting/driver_Grace"
data.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/data/cbc_for_seqNorm.xlsx")

cell.data.orig<-read.xlsx(data.filepath,sheetIndex=1,check.names=F)
cell.data<-cell.data.orig
```

Third, we adjust the gene expression data based on the cell differentials.
```{r eval=FALSE}
######################################################
# This code chunk only needs to be run once.
######################################################

#home.dir<-"/home/xy48"
# for each gene, adjust the data using the cell differentials
#cell.data<-clinic.matrix[,c("p_mono","p_eos","p_lymph","p_neut","p_baso")]
#cell.data["06S026","p_lymph"]<-25.3
#cell.data["04S050",]<-rep(NA,ncol(cell.data))
output.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/data")
rownames(cell.data)<-cell.data[,"sample_names"]
cell.data<-cell.data[colnames(fpkm),]
cell.data<-cell.data[,c("pmn","lymph", "mono", "eosin", "baso")]/matrix(rep(cell.data[,"wbc"],5),nrow=nrow(cell.data),byrow = F)

# adjust the gene expression data
my.data.matrix<-fpkm
anno.matrix<-anno.data
res.matrix<-numeric()
for(i in 1:nrow(my.data.matrix)){
  temp.data<-data.frame(gexp=t(my.data.matrix[i,]),mono=cell.data[,"mono"],eos=cell.data[,"eosin"],lym=cell.data[,"lymph"],pmn=cell.data[,"pmn"],baso=cell.data[,"baso"])
  colnames(temp.data)[1]<-"gexp"
  my.res<-lm(gexp~.,data=temp.data)$residuals
  res.matrix<-rbind(res.matrix,my.res)
}
rownames(res.matrix)<-rownames(my.data.matrix)
colnames(res.matrix)<-colnames(my.data.matrix)
rownames(anno.matrix)<-anno.matrix[,1]
cmd.out<-cbind(anno.matrix[rownames(res.matrix),],res.matrix)

output.subdir<-output.dir
if(file.exists(output.subdir)==F){
  dir.create(output.subdir)
}
output.filepath<-file.path(output.subdir,"DESeq2log2_celldiffadjusted_allsamples.txt")
write.table(cmd.out,file=output.filepath,append=F,sep="\t",row.names=F,col.names=T,quote=F)

output.filepath<-file.path(output.subdir,"DESeq2log2_celldiffadjusted_allsamples.RDS")
...
outliers.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/DESeq2_log2_outlierlist.RDS")
outlier.list<-readRDS(outliers.filepath,refhook = NULL)
...
# remove outliers
#fpkm<-fpkm[,colnames(fpkm)!="sarcoidosis_X235"]
fpkm<-fpkm[,colnames(fpkm)!=outlier.list]
...
output.filepath<-file.path(output.subdir,"DESeq2log2_celldiffadjusted_nooutliers.txt")
write.table(cmd.out,file=output.filepath,append=F,sep="\t",row.names=F,col.names=T,quote=F)

output.filepath<-file.path(output.subdir,"DESeq2log2_celldiffadjusted_nooutliers.RDS")
saveRDS(cmd.out,file=output.filepath,refhook = NULL)
...
# GSEA input files generation (adjusted_all samples)
...
deseq2.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/data/DESeq2log2_celldiffadjusted_allsamples.RDS")
my.data<-readRDS(deseq2.filepath,refhook = NULL)
...
fpkm.output.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/GSEA_knowngenes_log2DESeq2_adjusted/DESeq2_log2_sarconly_matched_all.RDS")
clinic.output.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/GSEA_knowngenes_log2DESeq2_adjusted/clinic_data_sarconly_matched_all.RDS")
fpkm.annot.output.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/GSEA_knowngenes_log2DESeq2_adjusted/DESeq2_log2_annot_sarconly_matched_all.RDS")
...
my_gsea_filecreate_binary_old<-function(fpkm.matrix,clinic.matrix,fpkm.matrix.anno,var.names,group1.values,group2.values,group1.name="group1",group2.name="group2",output.dir,suffix.name){
  #####################################################################################################################################################
  # Arguments:
  # 
  # gexp.filepath is the file path of the TPM matrix with first 5 columns being annotation for genes 
...
output.folder<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/GSEA_knowngenes_log2DESeq2_adjusted/all_samples_withID")
...
# GSEA input files generation (adjusted_no outliers)
...
fpkm.output.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/GSEA_knowngenes_log2DESeq2_adjusted/DESeq2_log2_sarconly_matched_nooutliers.RDS")
clinic.output.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/GSEA_knowngenes_log2DESeq2_adjusted/clinic_data_sarconly_matched_nooutliers.RDS")
fpkm.annot.output.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/GSEA_knowngenes_log2DESeq2_adjusted/DESeq2_log2_annot_sarconly_matched_nooutliers.RDS")
...
output.folder<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/Laura_Koth/GSEA_knowngenes_log2DESeq2_adjusted/no_outliers_withID")
