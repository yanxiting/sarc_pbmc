---
title: "WGCNA of SARC PBMC RNAseq data"
author: "Xiting Yan"
date: "10/02/2019"
output:
  html_notebook:
...
In this note, we adjust the gene expression to remove the effect of cell differentials first. Then we apply WGCNA onto the adjusted data to identify gene modules. 

We ran this notes on the workstation with files read from the mounted grace hpc scratch folders.

# Data adjustment for cell differentials

## No Log2 Transformation
We remove the effect of cell differentials using a linear regression model. 
```{r eval=FALSE}
######################################################
...
anno.matrix<-as.matrix(fpkm.matrix)[,1:5]
rownames(anno.matrix)<-paste(anno.matrix[,"Gene_Id"],"_",anno.matrix[,"Chr"],sep="")
rownames(fpkm.matrix)<-paste(anno.matrix[,"Gene_Id"],"_",anno.matrix[,"Chr"],sep="")
data.matrix<-fpkm.matrix[,6:ncol(fpkm.matrix)]
...
output.subdir<-file.path(output.dir,"data_adjusted2")
if(file.exists(output.subdir)==F){
  dir.create(output.subdir)
}
output.filepath<-file.path(output.subdir,"TPM_baseline_276_clean_celldiffadjusted_withannot.txt")
write.table(cmd.out,file=output.filepath,append=F,sep="\t",row.names=F,col.names=T,quote=F)
```

## With Log2 Transformation

We also adjusted the data with taking log2 transformation first. 
...
#home.dir<-"/home/xy48"
data.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38")
fpkm.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/data","log2TPM_baseline_276_clean_GRADSID.txt")
masterkey.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Data/GRADS Master Progress Key 6-13-16.xlsx")
clinic.filepath<-file.path(home.dir,"scratch/GRADS/SARC_results/Data_PBMC/clin_data_visit1_XY.csv")
...
output.filepath<-file.path(output.subdir,"log2TPM_baseline_276_clean_celldiffadjusted_withannot.txt")
write.table(cmd.out,file=output.filepath,append=F,sep="\t",row.names=F,col.names=T,quote=F)
```
...
output.dir<-file.path(home.dir,"scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/WGCNA_celladjusted2")
...
## Heatmap of correlation with module size
...
First, we load in the WGCNA results and data.
```{r}
...
data.filepath<-"/home/yanxiting/driver_Grace/scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/WGCNA_celladjusted2/CV0_compare/wgcna_cutheights.RData"
...
Second, we redraw the heatmap of the correlation between clinical traits and gene modules with the module size in it.
```{r fig.width=10,fig.height=12,cache=FALSE,fig.cap="Heatmap showing the corrleation between the gene modules identified by WGCNA with the clinical traits with gene module size."}
...
modNames=substring(names(MEs), 3)
output.subdir<-file.path("/home/yanxiting/driver_Grace/scratch/GRADS/SARC_results/Results_summary_PBMC_hg38/baseline/WGCNA_celladjusted2/CV0_compare",paste("ModuleMembership_GSPvalue_outliers_",i,"_withFEV1FVCratio",sep=""))
if(file.exists(output.subdir)==F){
dir.create(output.subdir)	
}
for(k in 1:length(modNames)){
...
output.filepath<-file.path(output.subdir,paste("WGCNA_celladjusted2_heatmap_trait_correlation_modules_outliers_",i,"_modulenames_def.txt",sep=""))
write.table(modulenames.def,file=output.filepath,append=F,sep="\t",col.names=T,row.names=F,quote=F)

output.filepath<-file.path(output.subdir,paste("WGCNA_celladjusted2_heatmap_trait_correlation_modules_outliers_",i,"_clean.eps",sep=""))
postscript(output.filepath,width=10,height=12,paper="special")
par(mar = c(8, 8, 3, 1));
labeledHeatmap(Matrix = temp.moduleTraitCor,
xLabels = rownames(my.datTraits),
#yLabels = names(MEs),
